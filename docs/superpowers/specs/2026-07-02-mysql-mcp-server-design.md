...
3. **语句类型分级**：AST 根节点映射为 SELECT / INSERT / UPDATE / DELETE / DDL / UTILITY（SHOW、DESCRIBE、EXPLAIN）六类，对照 `allowed_statements` 判定；映射表之外的一切语句类型（SET、GRANT、CALL、LOAD DATA、LOCK TABLES、存储过程/触发器 DDL 等）无条件拒绝——白名单式分类，新语法默认落在拒绝侧。
